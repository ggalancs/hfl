...
class TestFormatSize:
    ...
    def test_megabytes(self):
        ...
        result = format_size(50 * 1024 * 1024)  # 50 MB
        assert "MB" in result
        assert "50" in result
    ...
    def test_large_gigabytes(self):
        ...
        result = format_size(50 * 1024 * 1024 * 1024)  # 50 GB
        assert "GB" in result

    def test_fractional_gigabytes(self):
        """Test formatting fractional gigabyte sizes."""
        result = format_size(int(1.5 * 1024 * 1024 * 1024))  # 1.5 GB
        assert "GB" in result
...
class TestExtractParamsFromName:
    ...
    def test_extract_70b(self):
        ...
        result = extract_params_from_name("meta-llama/Llama-3.3-70B-Instruct")
        assert result == "70B"

    def test_extract_7b(self):
        """Test extracting 7B parameters."""
        result = extract_params_from_name("mistralai/Mistral-7B-Instruct-v0.3")
        assert result == "7B"

    def test_extract_1_5b(self):
        """Test extracting 1.5B parameters."""
        result = extract_params_from_name("microsoft/phi-1.5b")
        assert result == "1.5B"

    def test_extract_405b(self):
        """Test extracting 405B parameters."""
        result = extract_params_from_name("meta-llama/Llama-3.1-405B")
        assert result == "405B"

    def test_extract_from_suffix(self):
        """Test extracting from model-7b pattern."""
        result = extract_params_from_name("org/model-7b-chat")
        assert result == "7B"

    def test_no_params_found(self):
        """Test when no parameters can be extracted."""
        result = extract_params_from_name("org/some-model-without-params")
        assert result is None

    def test_lowercase_b(self):
        """Test extracting with lowercase 'b'."""
        result = extract_params_from_name("org/model-13b")
        assert result == "13B"

    def test_with_instruct_suffix(self):
        """Test extracting with -instruct suffix."""
        result = extract_params_from_name("org/model-70b-instruct")
        assert result == "70B"
