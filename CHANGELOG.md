...
- **Coding agents checked for real: `scripts/agent_check.py`.** Starts HFL
  ...
  marker).
- **`hfl import <file or folder>`: a model you already have, where it is.**
  For models downloaded by LM Studio, llama.cpp, `huggingface-cli` or by
  hand — a GGUF, or a folder of MLX or Hugging Face weights (`config.json`
  and `.safetensors`, with its tokenizer), served by the backend a pulled one
  would get and labelled with what its weights are (`4bit`, `BF16`), chat
  or embeddings as its config says: registered in
...
- **Search-triggered downloads crashed** with `quote_from_bytes() expected
  bytes`. The revision-pinning feature added a `--revision` option to `pull`;
  the search flow calls `pull()` as a plain function, so that option stayed at
  its `typer.OptionInfo` sentinel (truthy, non-str) and leaked into the Hub's
  URL quoting. `resolve()` now coerces any non-string revision to `None`, and
  the internal call passes `revision=None` explicitly. Regression test added;
  verified end-to-end (search → download → run).
